...
from allocation import (cube_root_size, huntington_hill,  # noqa: E402,F401
                        smallest_unit_size)
...
def house_size(rule, pops):
    """How many seats the House has under each rule.

    Only "fixed" is American: 435 is a number Congress picked in 1929 and never
    revisited. The other two are general rules and live in shared/allocation.py
    -- naming the smallest-unit rule after Wyoming is a US convention, not part
    of the rule.
    """
    if rule == "fixed":
        return HOUSE_SIZE_1929
    if rule == "cuberoot":
        return cube_root_size(pops)
    if rule == "wyoming":
        return smallest_unit_size(pops)
