...
def pick_field(rec, names):
    """First of `names` present in a .dbf record, or None.

    A list rather than one name because shapefiles disagree with themselves
    about what the same column is called between vintages -- the US census
    files alone use STATE and STATEFP for the same thing.
    """
    for k in names:
        if k in rec:
            return rec[k]
...
def main():
    ...
    ap.add_argument("--key-field", nargs="*",
                    default=["STATEFP", "STATE", "STATE_FIPS"],
                    help="candidate .dbf column names identifying the region "
                         "each polygon belongs to; the first one present wins. "
                         "Defaults suit US census files.")
    ap.add_argument("--keep", nargs="*", default=None,
                    help="values of the key field to keep; default keeps "
                         "everything")
    ap.add_argument("--fips", nargs="*", default=None,
                    help=argparse.SUPPRESS)  # old name for --keep
    ...
    if not rings:
        raise SystemExit("no rings selected -- check --keep against the .dbf")

    n = write_dat(rings, Path(args.out))
    verts = sum(len(r) for r in rings)
    print(f"{args.out}: {n} rings, {verts} vertices, "
          f"{len(kept)} region(s): {' '.join(sorted(x for x in kept if x))}")
    return 0
